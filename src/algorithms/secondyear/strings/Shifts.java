...
public class Shifts {
    ...
    public void solve() throws IOException {
        ...
        if (!check) {
            ...
            ans = str.substring(pos, str.length()) + str.substring(0, pos);
            out.print(ans);
        }
    ...
    public void run() {
        try {
            in = new FastScanner(new File("shifts.in"));
            out = new PrintWriter(new File("shifts.out"));
            solve();
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
